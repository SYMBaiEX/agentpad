...
`AGENTPAD_OPENAI_ACTIONS_PER_MINUTE` so an active browser cannot accidentally
run an unbounded OpenAI action loop.

The agent loop ticks at `AGENTPAD_AGENT_TICK_MS` (16ms by default). OpenAI has
...
The two agents connect through one AgentPad controller hub endpoint:
...
- AgentPad virtual controller state over WebSocket
